...
public class IMUSimulator : MonoBehaviour
{
    ...
    public void SetupAndSaveData()
    {
        ...
        if (File.Exists(_path))
        {
            ...
            while (reader.ReadLine() is { } line)
            {
                ...
                if (activeManeuver != null)
                {
                    ...
                    if (!float.TryParse(bits[8], NumberStyles.Any, CultureInfo.InvariantCulture,
                            out newSample.angles.x))
                    {
                        Debug.Log("x-angle wrong format");
                    }

                    if (!float.TryParse(bits[9], NumberStyles.Any, CultureInfo.InvariantCulture,
                            out newSample.angles.z))
                    {
                        Debug.Log("y-angle wrong format");
                    }

                    if (!float.TryParse(bits[10], NumberStyles.Any, CultureInfo.InvariantCulture,
                            out newSample.angles.y))
                    {
        ...
        {
            Debug.LogError("File does not exist in path:"+_path+" Please provide correct path to the text file.");
        }
    }
